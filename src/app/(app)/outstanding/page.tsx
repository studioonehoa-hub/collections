...
export default async function OutstandingPage({
  ...
  return (
    <section>
      <h2 className="text-[17px] font-semibold text-gray-900">Outstanding Report</h2>
      <p className="text-gray-500 text-[12.5px] mb-4">Billed vs paid per unit, per month.</p>
      ...
      <form method="GET" className="flex gap-2 flex-wrap items-center mb-4">
        <input
          ...
          className="max-w-[160px] rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 outline-none focus:border-blue-700"
        />
        <select
          name="filter"
          defaultValue={filter ?? "all"}
          className="max-w-[180px] rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 bg-white"
        >
          <option value="all">All units</option>
          <option value="unpaid">Unpaid only</option>
        </select>
        <button type="submit" className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold">
          View
        </button>
        <Link
          href={`/outstanding/export?period=${encodeURIComponent(period)}&filter=${filter ?? "all"}`}
          className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold ml-auto"
        >
          ⬇ Export CSV
        </Link>
      </form>
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.unit_no} className="border-b border-gray-100 last:border-0">
                  ...
                  <td className={`px-3 py-2 text-right tabular-nums ${balance > 0.005 ? "text-red-700" : ""}`}>
                    {formatPhp(balance)}
                  </td>
