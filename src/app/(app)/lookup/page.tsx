...
export default async function LookupPage({
  ...
  return (
    <section>
      <h2 className="text-[17px] font-semibold text-gray-900">Resident Lookup</h2>
      ...
      <form className="flex gap-2 mb-4 flex-wrap" action="/lookup">
        ...
        {canEdit && (
          <Link
            href="/residents/new"
            className="rounded-lg border border-gray-300 text-gray-900 text-sm font-semibold px-4 py-2 ml-auto"
          >
            + Add resident
          </Link>
        )}
      ...
      {resident && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <table className="w-full text-[13px]">
            <tbody>
              <tr>
                <td className="text-gray-500 py-1 pr-4 w-48 align-top">Group / dues</td>
                <td className="py-1">{groupLabel}</td>
              </tr>
              <tr>
                <td className="text-gray-500 py-1 pr-4 align-top">Billing contact 1</td>
                <td className="py-1">{resident.billing_contact_1 || "—"}</td>
              </tr>
              <tr>
                <td className="text-gray-500 py-1 pr-4 align-top">Billing contact 2</td>
                <td className="py-1">{resident.billing_contact_2 || "—"}</td>
              </tr>
              <tr>
                <td className="text-gray-500 py-1 pr-4 align-top">Status</td>
                <td className="py-1">
                  <span
          ...
          <div className="mt-3 flex gap-2 flex-wrap">
            <Link
              ...
              className="rounded-lg border border-gray-300 text-xs font-semibold px-3 py-1.5"
            >
              View ledger
            </Link>
            <Link
